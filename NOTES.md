...
- **타임라인 = Structured식 순차 레이아웃** (`renderTimelineView`/`seqCard`): 24시간 고정 그리드가 아니라 task 수만큼 늘어나는 카드 목록. 빈 시간은 점선 커넥터(45분 이상이면 `50m free` 라벨, 클릭하면 그 시간으로 새 task). 카드에 시작–끝 시간 표기
- **타임라인 블록 높이 = 소요시간 비례** (`tlHeight`): `dur × 1.7px`, 최소 44px / 최대 260px. 레일 위 컬러 블록(`.tls-dot`, 캡슐형)이 `align-self:stretch`로 항목 높이를 채우므로 짧은 일(≈35분 이하)은 카드 본문 높이(2줄 ≈ 64px)에 걸려 같은 크기로 보인다 — 의도된 가독성 하한
- **겹치는 시간대 표기**: 정렬된 timed 목록에서 `start < prevEnd`(또는 시작 시각이 같음)이면 두 블록 사이에 `.tls-ov` 마커(렌즈 도형 + `Tasks are overlapping · 30m`) 삽입, 양쪽 항목에 `.ov` 클래스로 앰버 링. 색은 `--defer`
- **타임라인 시간 라벨**: 좌측 거터에 블록 위=시작(`.tl-t0`), 아래=끝(`.tl-t1`). 다음 일정이 같은 시각에 바로 이어지면 끝 라벨 생략(중복 방지) — 드래그 중에도 두 라벨 모두 실시간 갱신
...
20. 근무 카드/리포트 UI 정돈 (sw 캐시 v7 — 폰에서 v6 style.css가 캐시로 미반영돼 깨져 보였던 문제 겸 해결):
...
22. **기분 아이콘 의미 통일** (sw 캐시 v9): Sad의 비구름, Tired의 달, Sick의 온도계를 각각 눈물 얼굴·하품 얼굴·아픈 얼굴로 교체. 감정보다 활동에 가까운 Party는 Excited 표정으로 바꾸고, 기존 `mood-party`/🥳 저장값은 계속 읽히도록 호환 처리
